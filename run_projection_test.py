...
N_SUBS = 45
REF = op.join(OUTROOT, 'ref_3.0.nii.gz') # reference space for all tests 
REF_SPC = tob.ImageSpace(REF) # same again, as object 
FACTOR = 5  # voxel subdivision factor for both methods 
EXPERIMENTS = ['naive', 'wm_cbf'] # wb_cbf corresponds to 2-component signal 
SIGNALS = ['flat', 'sine'] # signal distributions 
REFRESH = True # force recomputation of all data 
...
def make_activation(for_surf, mask):
    ...
    while not valid: 
        ...
        old_n = for_surf.tris[(for_surf.tris == at_idx).any(1),:].flatten()
        metric[old_n] = 1 
        old_n = set(old_n)
        for sig in [0.9,0.7,0.5,0.3,0.1]:
            new_n = for_surf.tris[np.isin(for_surf.tris, list(old_n)).any(1),:].flatten()
            update = set(new_n) - set(old_n)
            metric[list(update)] = sig
            old_n = new_n 
        final_set = (metric > 0)
        valid = mask[final_set].all()
    return metric 
...
def make_fields(spc):        
    ...
    SINE_FIELD = FLAT_FIELD + 6 * (np.sin(i) + np.sin(j) + np.sin(k))
    WM_SINE_FIELD = WM_FLAT_FIELD + (2 * (np.sin(i) + np.sin(j) + np.sin(k)))
 
    return { 'flat': FLAT_FIELD, 'wm_flat': WM_FLAT_FIELD, 'sine': SINE_FIELD, 
            'wm_sine': WM_SINE_FIELD } 
...
def make_surf_truths(spc, surf):
    ...
    FLAT = interpn((i,j,k), field_dict['flat'].reshape(spc.size), interp_points)
    SINE = interpn((i,j,k), field_dict['sine'].reshape(spc.size), interp_points)
    return { 'flat': FLAT, 'sine': SINE }
...
def make_volume_truths(pvs, spc):
    ...
    return { 'flat': FLAT, 'sine': SINE,  
              'wm_flat': WM_FLAT, 'wm_sine': WM_SINE, }
...
def test_subject(data_dict, surf_dict, subid, exp, surf_paths):
    ...
    if exp == 'naive':
        ...
        v2n_tob_mat_edge = projector.vol2surf_matrix(edge_scale=True)

    # Volume-node, with edge effects
    else:  
        v2n_tob_mat = projector.vol2node_matrix(edge_scale=False)
        n2v_tob_mat = projector.node2vol_matrix(edge_scale=True)
        v2n_tob_mat_edge = projector.vol2node_matrix(edge_scale=True)
    ...
    v2n_wb_mat = wb_projection.wb_v2n_matrix(wb_mat, False)
    n2v_wb_mat = wb_projection.wb_n2v_matrix(wb_mat)
    v2n_wb_mat_edge = wb_projection.wb_v2n_matrix(wb_mat, True)
    ...
    for key in SIGNALS: 
        ...
        path = pathroot + '_v2n_tob_edge.func.gii'
        if not op.exists(path) or REFRESH:
            v2n_tob_edge = v2n_tob_mat_edge.dot(vol)
            savefunc(v2n_tob_edge, path)
        v2n_tob_edge = loadfunc(path)  
        ...
        path = pathroot + '_v2n2v_tob_edge.nii.gz'
        if not op.exists(path) or REFRESH:
            v2n2v_tob_edge = n2v_tob_mat.dot(v2n_tob_edge)
            REF_SPC.save_image(v2n2v_tob_edge, path)
        ...
        path = pathroot + '_v2n_wb_edge.func.gii'
        if not op.exists(path) or REFRESH:
            v2n_wb_edge = v2n_wb_mat_edge.dot(vol)
            savefunc(v2n_wb_edge, path)
        v2n_wb_edge = loadfunc(path)
        ...
        path = pathroot + '_v2n2v_wb_edge.nii.gz'
        if not op.exists(path) or REFRESH:
            v2n2v_wb_edge = n2v_wb_mat.dot(v2n_wb_edge)
            REF_SPC.save_image(v2n2v_wb_edge, path)
        ...
        path = pathroot + '_n2v2n_tob_edge.func.gii'
        if not op.exists(path) or REFRESH:
            s2v2n_tob_edge = v2n_tob_mat_edge.dot(n2v_tob)
            savefunc(s2v2n_tob_edge, path)
        ...
        path = pathroot + '_n2v2n_wb_edge.func.gii'
        if not op.exists(path) or REFRESH:
            s2v2n_wb_edge = v2n_wb_mat_edge.dot(n2v_wb)
            savefunc(s2v2n_wb_edge, path)
    ...
    for exp in EXPERIMENTS:
        for idx, subid in enumerate(SUBIDS()[:N_SUBS]):
            ...
            if not op.exists(p_name):
                ...
                proj = tob.projection.Projector(L_hemi, REF_SPC, factor=FACTOR, cores=14)
                proj.save(p_name)
            else: 
            ...
            vol_truths = make_volume_truths(proj.pvs().reshape(-1,3), REF_SPC)
            ...
            # Prepare the surface mask which excludes areas with thickness < 2mm 
            smask_path = op.join(outdir, 'smask.func.gii')
            if not op.exists(smask_path) or REFRESH:
