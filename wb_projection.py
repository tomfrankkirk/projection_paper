...
def wb_v2n_matrix(wb_mat, edge_scale=False):
    """
    Produce wb_commands volume to surface projection.

    Args:
        wb_mat: scipy sparse matrix for volume to surface. 

    Returns:
        scipy sparse matrix for volume to surface
    """

    v2n_wb_mat = sparse_normalise(wb_mat, 1)
        
    if edge_scale: 
        gm_pv = wb_mat.sum(0).A.flatten()
        gm_pv = gm_pv / gm_pv.max()        
        brain = (gm_pv > 1e-3)
        upweight = np.ones(gm_pv.shape)
        upweight[brain] = 1 / gm_pv[brain]
        v2n_wb_mat.data *= np.take(upweight, v2n_wb_mat.indices)
